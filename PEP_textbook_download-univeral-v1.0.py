...
def open_github_site():
    webbrowser.open("https://github.com/lueduodezhe/PEP_textbook_download/tree/universal_textbook")

def open_web():
    webbrowser.open("http://www.nmketang.com/")
...
def download_images(page, ID, code):
    ...
    for a in range(page):
        url = f"http://pic.nmketang.com/dmt/diandu{code}/file/book/{ID}/"+str(a+1)+".jpg?v=2022_1"
...
# 创建窗口
window = tk.Tk()
window.title("冷门电子课本下载器v1.0(不支持语文、英语学科)")
# 设置窗口大小
