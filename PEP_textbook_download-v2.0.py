...
def convert_to_pdf(pdf_path):
    ...
    cwd = os.getcwd()
    messagebox.showinfo("当前工作目录", cwd)
...
def start_conversion():
    ...
    ID = entry_ID.get()
    code = entry_code.get()
    pdf_name = entry_pdf_name.get() + '.pdf'
...
label_pdf_name = tk.Label(window, text="PDF文件名:")
label_pdf_name.grid(row=3, column=0, padx=5, pady=5)
entry_pdf_name = tk.Entry(window)
entry_pdf_name.grid(row=3, column=1, padx=5, pady=5)
